...
def caculate_metric(pred_y, labels, pred_prob):
    ...
    pred_prob = pred_prob.detach().numpy().tolist()
    ...
    return metric, roc_data, prc_data, AUC1, fpr1, tpr1
...
if __name__ == '__main__':
    input = pd.read_csv('6mA.csv')
    prob_y = input.iloc[0, :]
    labels = input.iloc[1, :]
    pred_prob = 1 - input.iloc[2, :]
    caculate_metric(prob_y, labels, pred_prob)
